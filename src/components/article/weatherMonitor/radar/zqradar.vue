      ...
      <div class="left-foshan pull-left">
        <div class="lefttop-foshan">
          ...
          <div class="selectlist">
            <Select v-model="model2" @on-change="selectchange" size="small" style="width:200px">
              <Option v-for="item in cityListdatime" :value="item" :key="item">{{ item }}</Option>
            </Select>
          </div>
        </div>
          ...
          <div class="container" ref='mypp'>
            <div class="swiper-container" >
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item, index) in imgArrayf" :key="item.index" v-model="value2">
                  <img :src="item" alt="">
                </div>
              </div>
              ...
              <em @click="ImgClickBtn">
                <img v-if="btnimgshow" src="../img/ldstop.jpg" alt="">
                <img v-else src="../img/ldNext.jpg" alt="">
              </em>
            </div>
          </div>
        </div>
...
import { GetRaBar } from 'api/monitor'
  ...
  methods: {
    ...
    _GetRaBar() {
      GetRaBar().then(res => {
        console.log(res)
        if (res) {
          var imgarrf = res.data.ZhaoQing
          this.imgarrtemp = res.data.ZhaoQing
          for (var i = 0; i < imgarrf.length; i++) {
            var imgUrlarrayf = `http://10.0.1.184${imgarrf[i].path}`
            this.imgdatetime = imgarrf[i].datetime
            this.imgArrayf.push(imgUrlarrayf)
            this.cityListdatime.push(this.imgdatetime)
            // console.log(this.imgArrayf)
          }
          this.$nextTick(() => {
            this.init();
          });
     ...
     // 点击暂停开始
    ImgClickBtn () {
      if (this.btnimgshow === true) {
        this.btnimgshow = false
        this.mySwiper.autoplay.stop()
      } else {
        this.btnimgshow = true
        this.mySwiper.autoplay.start()
      }
    },
    // 点击选择时间加载同一时间的轮播图
    selectchange (val) {
      let index = this.imgarrtemp.findIndex(item => item.datetime === val)
...
em {
  width: 68px;
  height: 33px;
  background: url('../img/ldNext.jpg') no-repeat;
  float: right;
  cursor: pointer;
  position: relative;
  top: 22px;
