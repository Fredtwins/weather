    ...
    <div class="inside">
      <div class="nav-area">
        <template v-for="(item,index) in chartList">
          <span :key="'nav-'+item.id" :class="['nav-button',{'current':current===index}]" @click="switchChart(index)">{{item.name}}</span>
        </template>
      </div>
      <div class="chart-area">
        <template v-for="(item,index) in chartList">
          <div :key="'chart-'+item.id" :id="item.id" :class="['chart-box',{'current':current===index}]"></div>
        </template>
      </div>
      <div class="text-area" v-if="current===2" v-html="text"></div>
    </div>
  </div>
</template>
<script>
  ...
  import { getEcologyData } from 'api/article'
  ...
  export default {
    ...
    methods: {
      ...
      _getData () {
        getEcologyData().then(res => {
          if (res.code === ERR_OK) {
            let dataSource = []
            dataSource[0] = res.data.real // 实况数据
            dataSource[1] = res.data.average // 平均值数据
            dataSource[2] = res.data.forecast // 预测数据
      ...
      setCharts (data) {
        ...
        this.chartList.forEach(item => {
          ...
          chartElement.setOption(options[item.dataName])
        })
      }
    }
  }
